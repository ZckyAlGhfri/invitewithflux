...
export default function Event({ data, timeLeft }) {
  ...
  const EventCard = ({ title, date, time, loc, link }) => (
    <div className="group relative bg-stone-900 p-10 border-t-4 border-amber-500 hover:bg-amber-600 transition-all duration-500 h-full flex flex-col">
      ...
      <div className="flex items-start gap-4 mb-10">
        ...
        <div className="flex flex-col">
          <span className="text-lg font-bold text-amber-500 group-hover:text-white transition-colors">{getMonthYear(date)}</span>
            ...
            <div className="grid grid-cols-2 gap-4">
              {[{l:'Days', v:timeLeft.days}, {l:'Hrs', v:timeLeft.hours}, {l:'Min', v:timeLeft.minutes}, {l:'Sec', v:timeLeft.seconds}].map((t, i) => (
                <div key={i} className="bg-stone-900 p-6 flex flex-col border border-white/5">
                  <span className="text-3xl font-black text-amber-500">{String(t.v).padStart(2, '0')}</span>
