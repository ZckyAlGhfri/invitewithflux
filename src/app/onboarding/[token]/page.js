...
const themeOptions = [
  ...
  // HANDLER LOVE STORY (KEBAL ERROR)
  const handleLoveStoryChange = (index, field, value) => {
    const newStory = [...(formData.loveStory || [])]; 
    newStory[index][field] = value;
    setFormData({ ...formData, loveStory: newStory });
  };
  const addLoveStory = () => setFormData({ 
    ...formData, 
    loveStory: [...(formData.loveStory || []), { year: '2020', title: 'Pertama Bertemu', text: '' }] 
  });
  const removeLoveStory = (index) => setFormData({ 
    ...formData, 
    loveStory: (formData.loveStory || []).filter((_, i) => i !== index) 
  });
  const handleQuotePreset = (e) => { if(e.target.value) setFormData({ ...formData, quotes: e.target.value }); };

  // STATE BARU: Logika "Skip / Lewati Dulu"
  ...
  const handleUpdateGaleriArray = (index, url) => {
    const newGaleri = [...formData.fotoGaleri];
    newGaleri[index] = url;
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // PEMBERSIHAN DATA: Jika di-skip, paksa datanya kosong agar aman di database
      ...
      // Hanya upload jika TIDAK di-skip
            ...
            <div className="space-y-12 animate-[fadeIn_0.3s_ease-out]">
              ...
              <div>
                <div className="flex flex-col sm:flex-row sm:items-center justify-between border-b border-slate-100 pb-3 mb-5 gap-3">
                  ...
                  <label className="flex items-center gap-2 cursor-pointer bg-slate-50 px-3 py-1.5 rounded-lg border border-slate-200 w-fit hover:bg-slate-100 transition-colors">
                    <input type="checkbox" checked={skip.wanita} onChange={(e) => setSkip({...skip, wanita: e.target.checked})} className="w-4 h-4 accent-amber-500 rounded cursor-pointer" />
                    <span className="text-[10px] sm:text-xs font-bold text-slate-500 uppercase tracking-widest">Lewati Bagian Ini</span>
                  </label>
                </div>
              ...
              <div>
                <div className="flex flex-col sm:flex-row sm:items-center justify-between border-b border-slate-100 pb-3 mb-5 gap-3">
                  ...
                  <label className="flex items-center gap-2 cursor-pointer bg-slate-50 px-3 py-1.5 rounded-lg border border-slate-200 w-fit hover:bg-slate-100 transition-colors">
                    <input type="checkbox" checked={skip.pria} onChange={(e) => setSkip({...skip, pria: e.target.checked})} className="w-4 h-4 accent-amber-500 rounded cursor-pointer" />
                    <span className="text-[10px] sm:text-xs font-bold text-slate-500 uppercase tracking-widest">Lewati Bagian Ini</span>
                  </label>
                </div>
          ...
          {step === 3 && (
            <div className="space-y-10 animate-[fadeIn_0.3s_ease-out]">
              <div>
                ...
                <p className="text-xs text-slate-500 mb-4">Jika dilewati, undangan akan menggunakan siluet bunga elegan.</p>
              ...
              {isPremiumOrAbove && (
                <div>
                  <h3 className="font-bold text-slate-800 border-b border-slate-100 pb-3 mb-5">Galeri Foto Pre-Wedding</h3>
                  <div className="bg-slate-50 p-4 sm:p-5 rounded-2xl border border-slate-200 flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
                    <div><h4 className="font-bold text-slate-700 text-sm">Jumlah Foto Galeri</h4><p className="text-[11px] text-slate-500 mt-1">Maksimal: {maxFoto} foto</p></div>
                    <div className="flex items-center gap-3 bg-white p-2 rounded-xl border border-slate-200 shadow-sm w-fit"><button type="button" onClick={() => setJumlahFoto(Math.max(1, jumlahFoto - 1))} className="w-8 h-8 rounded-lg bg-slate-50 border border-slate-200 text-slate-600 hover:bg-slate-100 font-bold">-</button><span className="w-6 text-center font-bold text-slate-800">{jumlahFoto}</span><button type="button" onClick={() => setJumlahFoto(Math.min(maxFoto, jumlahFoto + 1))} className="w-8 h-8 rounded-lg bg-slate-50 border border-slate-200 text-slate-600 hover:bg-slate-100 font-bold">+</button></div>
                  </div>
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
                    {Array.from({ length: jumlahFoto }).map((_, index) => (
                      <div key={index} className="bg-white p-4 sm:p-5 rounded-2xl border border-slate-200 shadow-sm relative group"><div className="absolute -top-2 -left-2 w-6 h-6 bg-amber-500 text-white text-[10px] font-bold flex items-center justify-center rounded-full border-2 border-white z-10">{index + 1}</div><ImageUploader label={`Foto Galeri ${index + 1} (Crop 3:4)`} currentImage={formData.fotoGaleri ? formData.fotoGaleri[index] : null} onUploadSuccess={(url) => handleUpdateGaleriArray(index, url)} /></div>
                    ))}
                  </div>
                </div>
              )}
            ...
            <div className="space-y-8 animate-[fadeIn_0.3s_ease-out]">
              ...
              <div className="p-5 sm:p-6 bg-slate-50 rounded-2xl border border-slate-200">
                <div className="flex flex-col sm:flex-row sm:items-center justify-between mb-5 gap-3">
                  <h3 className="font-bold text-slate-700 flex items-center gap-2">💍 Akad Nikah</h3>
                  <label className="flex items-center gap-2 cursor-pointer bg-white px-3 py-1.5 rounded-lg border border-slate-200 w-fit hover:bg-slate-50 transition-colors">
                    <input type="checkbox" checked={skip.akad} onChange={(e) => setSkip({...skip, akad: e.target.checked})} className="w-4 h-4 accent-amber-500 rounded cursor-pointer" />
                    <span className="text-[10px] sm:text-xs font-bold text-slate-500 uppercase tracking-widest">Lewati Bagian Ini</span>
                  </label>
                </div>
                
                <div className={`space-y-4 transition-all duration-300 ${skip.akad ? 'opacity-40 grayscale-[50%] pointer-events-none' : ''}`}>
                  <div className="grid grid-cols-2 gap-4">
                    <div><label className="block text-[11px] font-bold text-slate-500 uppercase tracking-widest mb-1.5">Tanggal</label><input type="date" name="tanggalAkad" required={!skip.akad} disabled={skip.akad} value={formData.tanggalAkad} onChange={handleChange} className="w-full px-4 py-3 bg-white border border-slate-200 rounded-xl outline-none focus:border-amber-400 text-sm disabled:bg-slate-100" /></div>
                    <div><label className="block text-[11px] font-bold text-slate-500 uppercase tracking-widest mb-1.5">Waktu WIB</label><input type="time" name="waktuAkad" required={!skip.akad} disabled={skip.akad} value={formData.waktuAkad} onChange={handleChange} className="w-full px-4 py-3 bg-white border border-slate-200 rounded-xl outline-none focus:border-amber-400 text-sm disabled:bg-slate-100" /></div>
                  </div>
                  <div><label className="block text-[11px] font-bold text-slate-500 uppercase tracking-widest mb-1.5">Tempat / Gedung</label><input type="text" name="tempatAkad" required={!skip.akad} disabled={skip.akad} value={formData.tempatAkad} onChange={handleChange} placeholder="Gedung Pernikahan Flux" className="w-full px-4 py-3 bg-white border border-slate-200 rounded-xl outline-none focus:border-amber-400 text-sm disabled:bg-slate-100" /></div>
                  <div><label className="block text-[11px] font-bold text-slate-500 uppercase tracking-widest mb-1.5">Link Google Maps (URL)</label><input type="url" name="mapLinkAkad" disabled={skip.akad} value={formData.mapLinkAkad} onChange={handleChange} placeholder="https://maps.google.com/..." className="w-full px-4 py-3 bg-white border border-slate-200 rounded-xl outline-none focus:border-amber-400 text-xs text-blue-600 disabled:bg-slate-100" /></div>
                </div>
              </div>
              ...
              <div className="p-5 sm:p-6 bg-slate-50 rounded-2xl border border-slate-200">
                <div className="flex flex-col sm:flex-row sm:items-center justify-between mb-5 gap-3">
                  <h3 className="font-bold text-slate-700 flex items-center gap-2">🎉 Resepsi</h3>
                  <label className="flex items-center gap-2 cursor-pointer bg-white px-3 py-1.5 rounded-lg border border-slate-200 w-fit hover:bg-slate-50 transition-colors">
                    <input type="checkbox" checked={skip.resepsi} onChange={(e) => setSkip({...skip, resepsi: e.target.checked})} className="w-4 h-4 accent-amber-500 rounded cursor-pointer" />
                    <span className="text-[10px] sm:text-xs font-bold text-slate-500 uppercase tracking-widest">Lewati Bagian Ini</span>
                  </label>
                </div>

                <div className={`space-y-4 transition-all duration-300 ${skip.resepsi ? 'opacity-40 grayscale-[50%] pointer-events-none' : ''}`}>
                  <div className="grid grid-cols-2 gap-4">
                    <div><label className="block text-[11px] font-bold text-slate-500 uppercase tracking-widest mb-1.5">Tanggal</label><input type="date" name="tanggalResepsi" required={!skip.resepsi} disabled={skip.resepsi} value={formData.tanggalResepsi} onChange={handleChange} className="w-full px-4 py-3 bg-white border border-slate-200 rounded-xl outline-none focus:border-amber-400 text-sm disabled:bg-slate-100" /></div>
                    <div><label className="block text-[11px] font-bold text-slate-500 uppercase tracking-widest mb-1.5">Waktu WIB</label><input type="time" name="waktuResepsi" required={!skip.resepsi} disabled={skip.resepsi} value={formData.waktuResepsi} onChange={handleChange} className="w-full px-4 py-3 bg-white border border-slate-200 rounded-xl outline-none focus:border-amber-400 text-sm disabled:bg-slate-100" /></div>
                  </div>
                  <div><label className="block text-[11px] font-bold text-slate-500 uppercase tracking-widest mb-1.5">Tempat / Gedung</label><input type="text" name="tempatResepsi" required={!skip.resepsi} disabled={skip.resepsi} value={formData.tempatResepsi} onChange={handleChange} placeholder="Gedung Pernikahan Flux" className="w-full px-4 py-3 bg-white border border-slate-200 rounded-xl outline-none focus:border-amber-400 text-sm disabled:bg-slate-100" /></div>
                  <div><label className="block text-[11px] font-bold text-slate-500 uppercase tracking-widest mb-1.5">Link Google Maps (URL)</label><input type="url" name="mapLinkResepsi" disabled={skip.resepsi} value={formData.mapLinkResepsi} onChange={handleChange} placeholder="https://maps.google.com/..." className="w-full px-4 py-3 bg-white border border-slate-200 rounded-xl outline-none focus:border-amber-400 text-xs text-blue-600 disabled:bg-slate-100" /></div>
                </div>
              </div>

              {/* ICON BUILDER TATA TERTIB */}
              <div className="pt-8 border-t border-slate-100">
                <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-5 gap-2">
                  <div>
                    <h3 className="font-bold text-slate-800 mb-1">Tata Tertib Acara (House Rules)</h3>
                    <p className="text-xs text-slate-500">Pilih ikon dan ketik aturan untuk tamu undangan Anda (Opsional).</p>
                  </div>
                </div>
                <div className="space-y-3">
                  {formData.houseRules.map((rule, index) => (
                    <div key={index} className="flex flex-col sm:flex-row gap-3 items-start sm:items-center p-4 bg-slate-50 border border-slate-200 rounded-xl relative group">
                      ...
                      <input type="text" value={rule.text} onChange={(e) => handleHouseRuleChange(index, 'text', e.target.value)} placeholder="Contoh: Mohon hadir tepat waktu..." className="w-full p-3 bg-white border border-slate-200 rounded-lg outline-none focus:border-amber-400 text-sm" />
                      <button type="button" onClick={() => removeHouseRule(index)} className="p-3 text-red-400 hover:text-red-600 bg-white border border-slate-200 rounded-lg hover:bg-red-50 w-full sm:w-auto font-bold text-xs uppercase">Hapus</button>
                    </div>
                  ))}
                </div>
                <button type="button" onClick={addHouseRule} className="mt-4 px-5 py-2.5 bg-slate-900 text-white font-bold text-xs uppercase tracking-widest rounded-xl hover:bg-amber-600 shadow-lg active:scale-95">
                  + Tambah Aturan
                </button>
              </div>
         ...
         {/* STEP 5: KADO DIGITAL & LOVE STORY */}
          {step === 5 && isPremiumOrAbove && (
            <div className="space-y-8 animate-[fadeIn_0.3s_ease-out]">
              ...
              <div>
                <h3 className="font-bold text-slate-800 mb-1">Rekening Bank / E-Wallet</h3>
                <p className="text-xs text-slate-500 mb-5">Fasilitas kado digital (Cashless) untuk tamu undangan. Kosongkan jika tidak ingin memakai.</p>
                {formData.bankAccounts.map((bank, index) => (
                  <div key={index} className="flex flex-col sm:flex-row gap-4 items-start mb-4 bg-slate-50 p-5 rounded-2xl border border-slate-100 relative group">
                    <div className="flex-1 w-full space-y-3">
                      <div><label className="block text-[10px] font-bold text-slate-500 uppercase tracking-widest mb-1">Nama Bank (BCA/DANA)</label><input type="text" value={bank.bankName} onChange={(e) => handleBankChange(index, 'bankName', e.target.value)} className="w-full px-4 py-2.5 bg-white border border-slate-200 rounded-xl text-sm outline-none focus:border-amber-400" /></div>
                      <div><label className="block text-[10px] font-bold text-slate-500 uppercase tracking-widest mb-1">Nomor Rekening</label><input type="text" value={bank.accountNumber} onChange={(e) => handleBankChange(index, 'accountNumber', e.target.value)} className="w-full px-4 py-2.5 bg-white border border-slate-200 rounded-xl text-sm outline-none focus:border-amber-400" /></div>
                      <div><label className="block text-[10px] font-bold text-slate-500 uppercase tracking-widest mb-1">Atas Nama</label><input type="text" value={bank.accountName} onChange={(e) => handleBankChange(index, 'accountName', e.target.value)} className="w-full px-4 py-2.5 bg-white border border-slate-200 rounded-xl text-sm outline-none focus:border-amber-400" /></div>
                    </div>
                    {formData.bankAccounts.length > 1 && (<button type="button" onClick={() => removeBankAccount(index)} className="p-3 text-red-500 bg-white border border-slate-200 rounded-xl hover:bg-red-50 font-bold text-xs uppercase tracking-widest w-full sm:w-auto mt-2 sm:mt-6">Hapus</button>)}
                  </div>
                ))}
                <button type="button" onClick={addBankAccount} className="mt-2 text-sm font-bold text-amber-600 bg-amber-50 border border-amber-200 px-5 py-2.5 rounded-xl hover:bg-amber-100 transition-colors">+ Tambah Rekening Lain</button>
              </div>

              {/* --- BAGIAN 2: PERJALANAN CINTA (LOVE STORY) --- */}
              <div className="pt-10 mt-10 border-t border-slate-200">
                <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6">
                  <div>
                    <h3 className="font-bold text-slate-800 mb-1 flex items-center gap-2">💌 Perjalanan Cinta (Eksklusif)</h3>
                    <p className="text-xs text-slate-500">Ceritakan momen bersejarah hubungan Anda (Tahun Bertemu, Jadian, Tunangan).</p>
                  </div>
                  {clientTier === 'exclusive' && (
                    <button type="button" onClick={addLoveStory} className="px-5 py-2.5 bg-slate-900 text-white font-bold text-[10px] uppercase tracking-widest rounded-xl hover:bg-amber-600 transition-colors shadow-lg active:scale-95 whitespace-nowrap">
                      + Tambah Cerita
                    </button>
                  )}
                </div>
                ...
                {clientTier !== 'exclusive' ? (
                  <div className="text-center py-8 bg-slate-50 border border-slate-200 border-dashed rounded-2xl">
                    ...
                    <p className="text-slate-400 text-xs font-medium uppercase tracking-widest">Khusus Paket Exclusive</p>
                  </div>
                  ...
                  <div className="space-y-4">
                    {(formData.loveStory || []).map((story, index) => (
                      <div key={index} className="p-5 bg-slate-50 border border-slate-200 rounded-xl relative group">
                        <div className="absolute -left-2 -top-2 w-6 h-6 rounded-full bg-pink-500 text-white flex items-center justify-center text-[10px] font-bold border-2 border-white shadow-sm">{index + 1}</div>
                        <div className="flex flex-col sm:flex-row gap-3 mb-3">
                          <input type="text" value={story.year} onChange={(e) => handleLoveStoryChange(index, 'year', e.target.value)} placeholder="Tahun (Ex: 2021)" className="w-full sm:w-1/3 p-3 bg-white border border-slate-200 rounded-lg outline-none focus:border-amber-400 text-xs font-bold text-amber-600" />
                          <input type="text" value={story.title} onChange={(e) => handleLoveStoryChange(index, 'title', e.target.value)} placeholder="Judul (Ex: Pertama Bertemu)" className="w-full sm:w-2/3 p-3 bg-white border border-slate-200 rounded-lg outline-none focus:border-amber-400 text-sm font-bold text-slate-800" />
                        </div>
                        <textarea value={story.text} onChange={(e) => handleLoveStoryChange(index, 'text', e.target.value)} placeholder="Ceritakan kisah singkatnya di sini..." rows="2" className="w-full p-3 bg-white border border-slate-200 rounded-lg outline-none focus:border-amber-400 focus:bg-white transition-colors text-xs text-slate-600 mb-3"></textarea>
                        <button type="button" onClick={() => removeLoveStory(index)} className="text-red-500 hover:text-white bg-white hover:bg-red-500 border border-slate-200 p-2 rounded-lg w-full font-bold text-[10px] uppercase tracking-widest transition-colors">Hapus Cerita Ini</button>
                      </div>
                    ))}
                  </div>
                )}
          ...
          <div className="hidden sm:flex gap-4 pt-8 border-t border-slate-100">
            {step > 1 && <button type="button" onClick={() => {setStep(step - 1); window.scrollTo(0,0)}} className="px-6 py-4 rounded-xl font-bold text-slate-500 bg-slate-50 border border-slate-200 hover:bg-slate-100 transition-all w-1/3">Kembali</button>}
            <button type="submit" disabled={isSubmitting} className="w-full bg-slate-900 text-white py-4 rounded-xl font-bold tracking-widest uppercase text-sm hover:bg-amber-600 shadow-xl transition-all disabled:opacity-50 flex justify-center items-center gap-2">
              {isSubmitting ? <><div className="w-4 h-4 border-2 border-white/20 border-t-white rounded-full animate-spin"></div>Memproses Data...</> : step < totalSteps ? 'Lanjutkan' : 'Selesai & Buat Undangan'}
            </button>
          </div>
        </form>

        <div className="fixed bottom-0 left-0 w-full bg-white/95 backdrop-blur-md border-t border-slate-200 p-4 pb-5 flex gap-3 items-center justify-center z-[100] shadow-[0_-10px_30px_rgba(0,0,0,0.1)] sm:hidden animate-[slideUp_0.3s_ease-out]">
          {step > 1 && <button type="button" onClick={() => {setStep(step - 1); window.scrollTo(0,0)}} className="w-14 h-14 flex items-center justify-center rounded-full bg-slate-100 text-slate-600 border border-slate-200 active:scale-95 transition-all flex-shrink-0"><svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5"><path d="m15 18-6-6 6-6"/></svg></button>}
          <button type="button" onClick={() => document.querySelector('form').requestSubmit()} disabled={isSubmitting} className="flex-1 h-14 rounded-full font-bold text-white bg-amber-600 hover:bg-amber-700 shadow-xl shadow-amber-900/30 transition-all disabled:opacity-50 flex items-center justify-center gap-2 active:scale-95 text-sm tracking-widest uppercase">
            {isSubmitting ? <><div className="w-5 h-5 border-2 border-white/20 border-t-white rounded-full animate-spin"></div>Menyimpan...</> : step < totalSteps ? 'Lanjut' : 'Selesai & Simpan'}
          </button>
        </div>
