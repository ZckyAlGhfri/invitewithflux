...
export default function InvitationClient({ data, tamu }) {
  ...
  const colorVariant = data.theme_color || 'default'; 
  // =========================================================
    ...
    return (
      <div className="bg-[#FCFBF7] min-h-screen font-sans text-stone-800 overflow-x-hidden relative selection:bg-stone-200">
        ...
        <div className="fixed inset-0 pointer-events-none z-0 overflow-hidden">
          ...
          {[
            { top: '5%', left: '10%', size: 240, delay: 0, op: 0.07 },
            { top: '15%', left: '80%', size: 80, delay: 2, op: 0.12 },
            { top: '35%', left: '50%', size: 200, delay: 4, op: 0.08 },
            { top: '60%', left: '5%', size: 60, delay: 1, op: 0.15 },
            { top: '85%', left: '15%', size: 300, delay: 6, op: 0.1 },
            { top: '45%', left: '90%', size: 100, delay: 3, op: 0.1 },
            { top: '70%', left: '85%', size: 170, delay: 5, op: 0.07 },
            { top: '25%', left: '25%', size: 50, delay: 7, op: 0.18 },
            { top: '55%', left: '45%', size: 220, delay: 2, op: 0.07 },
            { top: '90%', left: '60%', size: 90, delay: 4, op: 0.12 },
          ].map((ornament, i) => (
            <motion.div key={i} className="absolute text-stone-500"
              style={{ top: ornament.top, left: ornament.left, width: ornament.size, height: ornament.size, opacity: ornament.op }}
              animate={{ y: [0, i % 2 === 0 ? -40 : 40, 0], x: [0, i % 3 === 0 ? 30 : -30, 0], rotate: [0, 360] }}
              transition={{ duration: 7 + i * 5, repeat: Infinity, ease: "linear", delay: ornament.delay }}
            >
        ...
        <div className="relative z-10">
          <ClassicCover data={data} isOpened={isOpened} handleOpen={handleOpen} tamu={tamu} imgSampul={imgSampul} colorVariant={colorVariant} />
          ...
          <div className="relative w-full">
            <ClassicHero data={data} imgSampul={imgSampul} colorVariant={colorVariant} />
            <ClassicCouple data={data} colorVariant={colorVariant} />
            {data.tier === 'exclusive' && <ClassicLoveStory data={data} colorVariant={colorVariant} />}
            <ClassicEvent data={data} timeLeft={timeLeft} colorVariant={colorVariant} />
            
            {data.tier !== 'basic' && (
              <>
                <ClassicGallery data={data} colorVariant={colorVariant} />
                <ClassicGift data={data} colorVariant={colorVariant} />
              </>
            )}
  ...
  if (currentTheme === 'modern') {
    return (
      <div className="bg-black min-h-screen font-sans selection:bg-amber-500 selection:text-white">
        <ModernCover data={data} isOpened={isOpened} handleOpen={handleOpen} tamu={tamu} imgSampul={imgSampul} colorVariant={colorVariant} />
        <audio ref={audioRef} src={data.music_url} loop />
        
        <button onClick={toggleAudio} className={`fixed top-6 right-6 z-[90] w-12 h-12 bg-white text-black flex items-center justify-center shadow-[4px_4px_0px_rgba(245,158,11,1)] transition-all hover:translate-x-[-2px] hover:translate-y-[-2px] hover:shadow-[6px_6px_0px_rgba(245,158,11,1)] active:translate-x-[2px] active:translate-y-[2px] active:shadow-none ${isOpened ? 'opacity-100' : 'opacity-0 pointer-events-none'}`}>
          {isPlaying ? <span className="text-[10px] font-black tracking-tighter uppercase">Stop</span> : <span className="text-[10px] font-black tracking-tighter uppercase">Play</span>}
        </button>

        <div className="relative w-full">
          <ModernHero data={data} imgSampul={imgSampul} colorVariant={colorVariant} />
          <ModernCouple data={data} colorVariant={colorVariant} />
          {data.tier === 'exclusive' && <ModernLoveStory data={data} colorVariant={colorVariant} />}
          <ModernEvent data={data} timeLeft={timeLeft} colorVariant={colorVariant} />
          
          {data.tier !== 'basic' && (
            <>
              <ModernGallery data={data} colorVariant={colorVariant} />
              <ModernGift data={data} colorVariant={colorVariant} />
            </>
          )}
  ...
  // === KAMUS WARNA KHUSUS FOOTER & NAVBAR LUXURY ===
  const luxuryStyles = {
    gold: { audio: "text-amber-600", footer: "text-amber-300", selection: "selection:bg-amber-100" },
    silver: { audio: "text-slate-500", footer: "text-slate-300", selection: "selection:bg-slate-200" }, // Footer diperterang jadi slate-300
    'rose-gold': { audio: "text-rose-600", footer: "text-rose-300", selection: "selection:bg-rose-100" }
  };
  const curLux = luxuryStyles[colorVariant] || luxuryStyles.gold;
  // =================================================

  return (
    <div className={`bg-slate-50 min-h-screen font-sans text-slate-800 overflow-x-hidden relative ${curLux.selection}`}>
      
      <Cover data={data} isOpened={isOpened} handleOpen={handleOpen} tamu={tamu} imgSampul={imgSampul} colorVariant={colorVariant} />

      <audio ref={audioRef} src={data.music_url} loop />
      <button onClick={toggleAudio} className={`fixed top-6 right-6 z-[90] w-12 h-12 bg-white/70 backdrop-blur-md border border-slate-200 rounded-full flex items-center justify-center shadow-xl transition-all hover:bg-white hover:scale-110 ${isPlaying ? 'animate-[spin_4s_linear_infinite]' : ''} ${isOpened ? 'opacity-100' : 'opacity-0 pointer-events-none'}`}>
        {/* Warna Icon Audio Disuntikkan Di Sini */}
        {isPlaying ? <svg className={`w-5 h-5 ${curLux.audio}`} fill="currentColor" viewBox="0 0 24 24"><path d="M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zm-1 14H9V8h2v8zm4 0h-2V8h2v8z"/></svg> : <svg className="w-5 h-5 text-slate-400 ml-1" fill="currentColor" viewBox="0 0 24 24"><path d="M8 5v14l11-7z"/></svg>}
      </button>

      <div className="relative w-full">
        <Hero data={data} imgSampul={imgSampul} colorVariant={colorVariant} />
        <Couple data={data} colorVariant={colorVariant} />
        
        {data.tier === 'exclusive' && <LoveStory data={data} colorVariant={colorVariant} />}
        
        <Event data={data} timeLeft={timeLeft} colorVariant={colorVariant} />
        
        {data.tier !== 'basic' && (
          <>
            <Gallery data={data} imgSampul={imgSampul} colorVariant={colorVariant} />
            <Gift data={data} colorVariant={colorVariant} />
          </>
        )}
        ...
        <footer className="bg-slate-950 text-white text-center py-24 pb-40 relative overflow-hidden">
            ...
            <Reveal direction="up">
              {/* Warna Judul Footer Disuntikkan Di Sini */}
              <h2 className={`text-5xl md:text-7xl font-serif italic mb-8 ${curLux.footer} drop-shadow-lg`}>{data.nama_wanita} & {data.nama_pria}</h2>
              ...
              <p className="text-xs font-bold text-white/60 tracking-[0.3em] uppercase mt-2">invitewithflux</p>
            </Reveal>
        </footer>
      </div>
