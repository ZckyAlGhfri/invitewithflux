...
export default function AdminEditPage() {
  ...
  useEffect(() => {
    async function fetchData() {
      ...
      if (invite.tier !== 'basic') {
        if (invite.bank_accounts && invite.bank_accounts.length > 0) {
          fetchedBanks = invite.bank_accounts.map(b => ({ bankName: b.bank_name, accountNumber: b.account_number, accountName: b.account_name }));
        }
      }

      const initialGaleri = invite.galleries 
        ? invite.galleries.sort((a, b) => a.position - b.position).map(g => g.image_url) 
        : [];

      // PARSING JSONB HOUSE RULES
      let parsedRules = [];
      if (invite.house_rules) {
        try { parsedRules = typeof invite.house_rules === 'string' ? JSON.parse(invite.house_rules) : invite.house_rules; } 
        catch (e) { parsedRules = []; }
      }

      // PARSING JSONB LOVE STORY (Taruh di bawah parsing house_rules)
      let parsedStory = [];
      if (invite.love_story) {
        try { parsedStory = typeof invite.love_story === 'string' ? JSON.parse(invite.love_story) : invite.love_story; } 
        catch (e) { parsedStory = []; }
      }
      ...
      setFormData({
        ...
        tanggalAkad: invite.tanggal_akad ? invite.tanggal_akad.split('T')[0] : '', 
        waktuAkad: invite.waktu_akad || '', tempatAkad: invite.tempat_akad || '', mapLinkAkad: invite.map_link_akad || '',
        tanggalResepsi: invite.tanggal_resepsi ? invite.tanggal_resepsi.split('T')[0] : '', 
        waktuResepsi: invite.waktu_resepsi || '', tempatResepsi: invite.tempat_resepsi || '', mapLinkResepsi: invite.map_link_resepsi || '',
  ...
  // TAB 7 DITAMBAHKAN UNTUK ADMIN
  const tabs = [
    { id: 1, label: 'Cover' }, { id: 2, label: 'Mempelai' }, { id: 3, label: 'Galeri' }, 
    { id: 4, label: 'Acara' }, { id: 5, label: 'Kado' }, { id: 6, label: 'Musik' },
    { id: 7, label: 'Personalisasi' }
  ];

  if (loading) return <div className="min-h-screen bg-slate-50 flex flex-col items-center justify-center font-bold text-slate-400"><div className="w-10 h-10 border-4 border-slate-200 border-t-amber-500 rounded-full animate-spin mb-4"></div>Menyiapkan Ruang Operasi...</div>;
          ...
          <div className="p-8 md:p-12 bg-white min-h-[500px]">
            ...
            {activeTab === 1 && (
              <div className="space-y-6 animate-[fadeIn_0.3s_ease-out]">
                <div className="border-b border-slate-100 pb-4 mb-6">
                  <h2 className="text-xl font-bold text-slate-800">Foto Sampul Depan</h2>
                  <p className="text-sm text-slate-500 mt-1">Foto ini akan menjadi layar penuh (Full Screen) saat tamu pertama kali membuka undangan.</p>
                </div>
                <ImageUploader label="Upload Foto (Crop 9:16 Portrait)" aspectRatio={9/16} currentImage={formData.fotoSampul} onUploadSuccess={(url) => setFormData({...formData, fotoSampul: url})} />
              </div>
            )}
            ...
            {activeTab === 7 && (
              <div className="space-y-10 animate-[fadeIn_0.3s_ease-out]">
                ...
                {/* KARTU 3: LOVE STORY (ADMIN BYPASS) */}
                <div className="bg-slate-50 p-6 md:p-8 rounded-[2rem] border border-slate-100 shadow-sm relative overflow-hidden mt-8">
                  ...
                  {formData.tier !== 'exclusive' && (
                     <div className="mb-6 text-center py-3 bg-amber-50 border border-amber-200 rounded-xl text-amber-700 text-xs font-bold uppercase tracking-widest">
                       ⚠️ Peringatan Admin: Paket saat ini bukan Exclusive. Fitur ini mungkin tidak tampil di undangan klien.
                     </div>
                  )}
